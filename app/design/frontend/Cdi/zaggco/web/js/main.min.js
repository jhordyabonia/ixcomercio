				    	...
				    	$.ajax({
							...
							success: function(resState) {
								...
								$.each(response.cities, function(iState, valState){
								    $(fieldCityCheckout).find('select').append("<option value='"+valState.traxId+"' parentId='"+valState.traxId+"' postalcode='"+valState.postalCode+"'>"+valState.name+"</option>");
								});
				    ...
				    $(obj).find('#fieldCityCheckout').on('change', function (e) {
						...
						$.ajax({
							...
							success: function(resCity) {
								...
								$(obj).find('input[name="postcode"]').val($(valCity).attr('postalCode'));
