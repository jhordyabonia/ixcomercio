...
class SearchFilterResource extends FilterResource
{
    ...
    public function apply(Select $select, Filter $filter, $callback) {
        ...
        $use_product_temp_table  = $this->_scopeConfig->getValue('mana_core/log/use_product_temp_table',ScopeInterface::SCOPE_STORE);
        $use_count_temp_table = $this->_scopeConfig->getValue('mana_core/log/use_count_temp_table',ScopeInterface::SCOPE_STORE);
        if($use_count_temp_table&&$use_count_temp_table){
            $select->joinInner(['search_result' => $this->resultTables[$key]],'e.entity_id = search_result.' . TemporaryStorage::FIELD_ENTITY_ID, []);
        }
    }
}