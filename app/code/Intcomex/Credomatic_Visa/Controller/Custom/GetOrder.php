...
class GetOrder extends \Magento\Framework\App\Action\Action
{
    ...
    */
    ...
    public function execute(){ 
        ...
        $arrayData = array();
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/credomatic_visa_request.log');
        ...
        try {
            ...
            $procesor_id = $this->_scopeConfig->getValue('payment/credomatic_visa/processor_id'.$post['cuotas']);
            
            $total = number_format($order->getData()[0]['grand_total'],2,".","");
            $hash = md5($order->getData()[0]['entity_id'].'|'.$total.'|'.$time.'|'.$this->_scopeConfig->getValue('payment/credomatic_visa/key'));
            $arrayData['key_id'] = $this->_scopeConfig->getValue('payment/credomatic_visa/key_id');
            $arrayData['hash'] = $hash;
            $arrayData['time'] = $time;
            $arrayData['processor_id'] = $procesor_id;
            $arrayData['amount'] = $total;
            $arrayData['orderid'] = $order->getData()[0]['entity_id'];
            $arrayData['gateway'] = $this->_scopeConfig->getValue('payment/credomatic_visa/url_gateway');
