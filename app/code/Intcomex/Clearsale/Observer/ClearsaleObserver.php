...
use \Clearsale\Integration\Observer\RequestOrder;

class ClearsaleObserver extends \Clearsale\Integration\Observer\ClearsaleObserver
{
        ...
        try
        {
            if (!empty($order)) {
                ...
                if ($isActive) {
                    ...
                    $this->logger->info('REQUEST: ' . json_encode($requestOrder));
                ...
                if ($isActive && $order->getStatus() == $analyzing_clearsale) {
                    ...
                    if ($authResponse) {
                        ...
                        if ($response->HttpCode == 200) {
                            ...
                            $this->logger->info('Order: ' . $order->getId() . ' Status ' . $order->getStatus());
                            if ($order->getStatus() == 'approved_clearsale') {
                                $this->_executeApprovalFlow($order);
                            } elseif ($order->getStatus() === 'denied_clearsale' || $order->getStatus() === 'canceled_clearsale') {
                                $this->_executeCanceledFlow($order);
                            }
    ...
    {
        $this->logger->info('Init Create Invoice: ' . $order->getId());
        if ($order->canInvoice()) {
     ...
     * @param $order
     */
    private function _executeCanceledFlow($order)
    {
