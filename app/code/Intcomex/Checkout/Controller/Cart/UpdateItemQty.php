...
namespace Intcomex\Checkout\Controller\Cart;
...
class UpdateItemQty extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/debug_update_cart.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info('Call method collectTotals in Quote');
            
        $this->checkoutSession->getQuote()->collectTotals()->save();        
        $this->jsonResponse();
        } catch (LocalizedException $e) {
		$this->jsonResponse($e->getMessage());
		$logger->info('Response ' . $this->jsonResponse($e->getMessage()));
        } catch (\Exception $e) {
            $this->logger->critical($e->getMessage());
	    $this->jsonResponse('Something went wrong while saving the page. Please refresh the page and try again.');
	    $logger->info('Response ' . $this->jsonResponse('Something went wrong while saving the page. Please refresh the page and try again.'));
