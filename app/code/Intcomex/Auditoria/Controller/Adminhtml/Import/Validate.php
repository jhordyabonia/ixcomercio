 ...
 */
namespace Intcomex\Auditoria\Controller\Adminhtml\Import;
        ...
        if (($handle = fopen($path, "r")) !== FALSE) {
            ...
            foreach ($csvFile as $key => $line) {
                if($key>0){
                    ...
                    $storeId = $storesRepository->get(trim($dataLine[1]))->getId();
                    $products->setStoreId($storeId);
                    $sku = $dataLine[0];
                    $productObj = $products->loadByAttribute('sku',trim($sku));

                    $price = $dataLine[2];
                        if($dataLine[2]==''||empty($dataLine[2])||$dataLine[2]>$dataLine[3]){
                            $price = $dataLine[3];
                        }
                    ...
                    if($precioReferencia!=''&&!empty($precioReferencia)){
                        ...
                        $this->logger->info(' No se puede evaluar  '.$sku.' No tiene precio referencia en: '.$dataLine[1]);
                    }
                }
            }
            if($errors!=''){
